...
> **本 Fork 重磅功能**：点击仪表盘顶部 **📉 Stats** 标签页，随时掌握你的 AI 模型用量与活跃趋势！

| 面板 | 功能说明 |
|------|----------|
| ⚡ **核心指标卡** | 累计消耗配额 · 单日峰值 · 当前连续活跃天数 · 历史最长连续天数 |
| 🗓️ **活跃度热力图** | **GitHub Contribution Graph Style** 365 天经典绿色格子墙，支持 **每日 / 每周 / 累计** 三种视图切换与悬浮详情 |
| 📈 **多模型使用趋势** | 基于 Chart.js 的平滑折线图，支持 **近 7 日 / 30 日** 筛选，可点击图例隐藏/显示特定模型 |
| 🍩 **模型用量占比** | 环形占比图，中心显示总消耗量，右侧清晰罗列各模型用量与百分比 |

> **数据说明**：基于每次配额轮询的剩余比例变化自动估算并持久化记录在 VS Code 全局状态中，安装使用后立即开始自动积累历史数据。
